...
const { parseDate } = require('../dateParser-lambda')
...
const fromMarcJson = async (object, datasource) => {
  ...
  try {
    ...
    if (callnum && callnum.value) {
      ...
      if (callnumSuffix[0] && callnumSuffix[0].match(/[0-9]{4}/)) {
        const dateRange = await parseDate(callnumSuffix)
        builder.add(fieldMapper.predicateFor('Date Range'), { literal: [dateRange.start, dateRange.end] }, 0, { path: 'fieldTag v' })
      }
