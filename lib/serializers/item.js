...
var fromMarcJson = (object, datasource) => {
  ...
  try {
    ...
    // because we'll need a record of this item existing, but no one downstream cares about its data
    let suppressionCheck = object.getSuppressedWithRationale()
    if (suppressionCheck.suppressed || !object.isResearchItem()) {
      builder.add(fieldMapper.predicateFor('Suppressed'), { literal: suppressionCheck.suppressed, type: 'xsd:boolean' }, 0, { path: suppressionCheck.rationale.join(', ') })
    ...
    if (itemType) {
      promises.push(CatalogItemType.byCode(itemType).then((itemType) => {
