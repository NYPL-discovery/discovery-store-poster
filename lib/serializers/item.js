...
var fromMarcJson = (object, datasource) => {
  ...
  try {
    return FieldMapping.initialize('resources').then((fieldMapper) => {
      ...
      builder.add('rdf:type', { id: 'nypl:Item' })
      ...
      if (isResearchItemType) {
        ...
        return lookupItemType.then(() => Promise.resolve(builder.statements))
      }

      var barcode = null
      if (object.barcode) barcode = { value: object.barcode, path: 'barcode' }
      else if (object.varField('876', ['p'])) barcode = { value: object.varField('876', ['p'])[0], path: '876 $p' }
      if (barcode) builder.add(fieldMapper.predicateFor('Identifier'), { id: `urn:barcode:${barcode.value}` }, 0, { path: barcode.path })

      // Callnum
      var callnum = null
      // log.debug('Call num: ', object.callNumber, object.varField('852', ['h']))
      ...
      if (callnum && callnum.value) builder.add(fieldMapper.predicateFor('Call number'), { literal: callnum.value }, { path: callnum.path })
      ...
      if (status) {
        ...
        var promise = (status.id ? Status.byId(status.id) : Status.byCode(status.code).then((s) => s[0]))
          .then((s) => {
            builder.add('bf:status', { id: `status:${s.id}`, label: s.literal('skos:prefLabel') }, 0, { path: status.path })
          })
      ...
      return Promise.all(promises).then(() => {
        ...
        console.log('location requestable? ', holdingLocation.id)

        // Does item status permit checkout?
        var itemAvailable = builder.get(fieldMapper.predicateFor('Availability')).object_id === 'status:a'
        ...
        // console.log('cat item type: ', builder.get('nypl:catalogItemType'))
        var itemTypeId = builder.get('nypl:catalogItemType') ? builder.get('nypl:catalogItemType').object_id.split(':')[1] : null
        var itemTypeRequestable = itemTypeId == null ? true : [2, 3, 4, 5, 6, 7, 8, 25, 26, 32, 33, 34, 35, 42, 43, 52, 53, 55, 57, 60, 61, 65, 67].indexOf(builder.get('nypl:catalogItemType').object_id.split) >= 0

        var rationale = { locationRequestable, itemTypeRequestable, accessMessagePermits, itemAvailable }
        var path = Object.keys(rationale).map((prop) => {
          return `${rationale[prop] ? '✅' : '❌'} ${prop}`
        }).join('; ')
        console.log('path: ', path)

        var requestable = locationRequestable &&
          itemTypeRequestable &&
          accessMessagePermits &&
          itemAvailable
