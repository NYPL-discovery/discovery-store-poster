...
var fromMarcJson = (object, datasource) => {
  ...
  try {
    ...
    if (location && sierraLocationMapping[location.value]) {
      ...
      log.warn('Location id not recognized: ' + location.value, { id: object.id, nyplSource: object.nyplSource })
    }
    ...
    if (accessMessage.code) {
      ...
      } else log.error('unmapped opac message? ' + accessMessage.code, { id: object.id, nyplSource: object.nyplSource })
    }
