...
const { getBibCallNum } = require('../serialization-utils')
...
const fromMarcJson = async (object, datasource, bib) => {
  if (!datasource) datasource = Datasource.byMarcJsonNyplSource(object.nyplSource)
  if (!datasource) {
  ...
  try {
    ...
    // add vol:
    builder.add(fieldMapper.predicateFor('Volume Range'), { literal: parsedVols }, 0, { path: 'fieldTag v' })

    const parsedDate = parseDate.checkCache(fieldtagv)
    if (parsedDate) {
      builder.add(fieldMapper.predicateFor('Date Range'), { literal: parsedDate }, 0, { path: 'fieldTag v' })
    }
    ...
    if (object.isNyplRecord() && object.recapCustomerCode) {
      // recap customer code was added to the item in the Discovery Hybrid Indexer
      customerCode = object.recapCustomerCode
      customerCodePath = 'scsb query'
    } else if (!object.isNyplRecord()) {
      customerCode = object.varField('900', ['b'])[0]
    ...
    // Extract Due date:
    const dueDate = object.status && object.status.duedate ? object.status.duedate.split('T')[0] : null
    if (dueDate) {
      builder.add(fieldMapper.predicateFor('Due date'), { literal: dueDate })
    }
