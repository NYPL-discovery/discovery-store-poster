...
var fromMarcJson = (object, datasource) => {
  ...
  try {
    ...
    if (object.isPartnerRecord()) itemType = 1
    ...
    if (object.location && object.location.code) {
      let mappedLocation = sierraLocationMapping[object.location.code]
    ...
    if (holdingLocationCollectionType === 'Branch') isResearchItem = false
    ...
    return Promise.all(promises).then(() => {
      ...
      var suppressionRationale = []

      // First, we'll make it suppressed if it's actually marked suppressed:
      if (object.suppressed) {
        suppressed = true
        suppressionRationale.push('suppressed')
      }
      // Next, we'll suppress it if catalogItemType is 50 (temporary item):
      if (catalogItemType && catalogItemType.id === 50) {
        suppressed = true
        suppressionRationale.push('catalogItemType')
      }
      // Next, we'll suppress it if fixed "Item Code 2" is 's', 'w', 'd', or 'p'
      if (['s', 'w', 'd', 'p'].indexOf(object.fixed('Item Code 2')) >= 0) {
        suppressed = true
        suppressionRationale.push('fixed "Item Code 2"')
      }
      builder.add(fieldMapper.predicateFor('Suppressed'), { literal: suppressed, type: 'xsd:boolean' }, 0, { path: suppressionRationale.join(', ') })
