...
var fromMarcJson = (object, datasource) => {
  ...
  try {
    return FieldMapping.initialize('resources').then((fieldMapper) => {
      ...
      builder.add('nypl:itemType', { id: `urn:itemtype:${branchOrResearch}` }, 0, { path: 'fixed "Item Type"' })
      ...
      if (object.fixed('OPAC Message')) {
        console.log('got it: ', JSON.stringify(object, null, 2))
        process.exit()
      }
      ...
      if (location) {
        promises.push(
          Location.byId(location.value).then((rec) => {
            if (rec) {
              builder.add(fieldMapper.predicateFor('Location'), { id: `loc:${rec.id}`, label: rec.label() }, 0, { path: location.path })
            }
          })
        )
      }
