  ...
  try {
    return FieldMapping.initialize('resources').then((fieldMapper) => {
      ...
      promises.push(extractElectronicResourcesFromBibMarc(object).then((resources) => {
        ...
        // TODO: Add as supplementaryContent even if not tagged Appendix if it looks like a URL! (decision made feb 16 2017)
          ...
          mapping.marcs.forEach((marc) => {
            ...
            if (values && values.length) {
              values.forEach((value) => {
                ...
                builder.add(mapping.predicate, { literal: value, label: marc.description }, null, { path })
              })
            }
          })
        })
      ...
      if (object.ldr()) {
        if (object.ldr().bibLevel) {
          promises.push(Lookup.initialize('bib-levels').then((lookup) => {
            ...
            if (bibLevel) builder.add(fieldMapper.predicateFor('Issuance'), { id: `urn:biblevel:${bibLevel}`, label: lookup.lookup(bibLevel) }, 0, { path: 'LDR/07' })
          }))
        }
        // Rec type
