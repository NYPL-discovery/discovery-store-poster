...
function _writeToKinesis (object) {
  return new Promise((resolve, reject) => {
    // Get unique subjectIds
    // (Conventionally, all should have same subject_id, but let's be careful..)
    var uniqueSubjectIds = Object.keys(object.statements.map((statement) => statement.subject_id).reduce((h, subjectId) => {
      h[subjectId] = true
      return h
    }, {}))

    var recordParams = {
      Records: uniqueSubjectIds.map((subjectId) => ({
        Data: type.toBuffer({ uri: subjectId, type: 'record' }),
