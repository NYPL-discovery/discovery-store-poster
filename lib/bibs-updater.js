...
class BibsUpdater {
  ...
  _writeToIndexDocumentQueue (batch) {
    ...
    if (config.kinesisWriteStream) {
      ...
      return (new NyplStreamsClient({ nyplDataApiClientBase: process.env['NYPL_API_BASE_URL'], logLevel: 'debug' })).write(config.kinesisWriteStream.stream, recs)
        .then(() => batch)
  ...
  bibByApi (nyplSource, id) {
    ...
    return nyplApi.bib(nyplSource, id)
      .then((bib) => {
        if (bib) bib = BibSierraRecord.from(bib)
        if (bib) return bib
