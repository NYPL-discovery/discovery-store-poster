...
BibsUpdater.threaded = (opts) => {
  if (cluster.isMaster) {
    ...
    // ; (new KafkaConsumer()).offsets(config.get('kafka.bibs.topic')).then((offsets) => run(offsets.min, opts.limit ? offsets.min + opts.limit : offsets.max))
    if (opts.limit) {
