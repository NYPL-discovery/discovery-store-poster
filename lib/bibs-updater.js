...
class BibsUpdater {
  ...
  saveToDisc (bib) {
    var path = `./tmp/cache/bibs/${bib.nyplSource}/${bib.id}.json`
  ...
  _processBibKinesis(bib, options) {
    this.extractStatements(bib)
      ...
      .then(object => {
        // save statements somewhere, currently uploading to s3
        // console.log(object);
        return (new s3upload).upload(object);
      })
      .then(function (err) {
  ...
  bib (id) {
    ...
    if (true) {
      console.log('path', `./tmp/cache/bibs/${nyplSource}/${id}.json`)
      return utils.readJson(`./tmp/cache/bibs/${nyplSource}/${id}.json`).then((bib) => {
