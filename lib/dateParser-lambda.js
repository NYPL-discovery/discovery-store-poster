...
// Takes an array of dates or a single date and returns a 2D array with the given date/s parsed using the ruby gem timetwister
const parseDate = async function (dates) {
  if (!Array.isArray(dates)) dates = [dates]
  const classifications = dates.map((date) => classifyDate(date))
  const preparsedDates = dates.map((date, i) => preparse(date, classifications[i]))
  try {
    const dateRanges = await timeTwist(preparsedDates)

    const parsedDates = dateRanges.map((ranges) => ranges.map((range) => [range.start, range.end]))
    return parsedDates
  } catch (e) {
    console.error(e)
  }
