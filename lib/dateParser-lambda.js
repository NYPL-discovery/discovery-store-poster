...
const parseDate = async function (date) {
  ...
  let preparsedDate = preparse(date, classifications)
  let dateRange
  try {
...
const classifyDate = (date) => {
  ...
  const yearRange = date.match(/\d{4}\/(\d{4}|\d{2})/g)
  if (yearRange && yearRange.length === 1) {
    classifications.push('twoYearRangeSolo')
  }
  // 1956/57-1981/82 (w 2 or 4 years after slash)
  if (date.match(/\d{4}\/(\d{4}|\d{2})-\d{4}\/(\d{4}|\d{2})/)) classifications.push('twoYearRangeMulti')
...
const soloRangeRemoveSlash = (range) => {
  range = range.match(/\d{4}\/(\d{4}|\d{2})/)[0]
