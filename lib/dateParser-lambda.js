  ...
  return rangesArray.map((rangesPerFieldtagv, rangesArrayIndex) => {
    ...
    return rangesPerFieldtagv.map((range) => {
      if (range[0] === null && range[1] === null) {
        ...
        // Dont return anything if range was [null, null] and nothing is returned from hailMary
      }
      return rangesPerFieldtagv
    })
  })
}
...
const hailMary = {
  matchExpression: /(?:19|20)\d{2}/,
  transform: function (range) {
    return range.match(this.matchExpression)[0]
  }
}
