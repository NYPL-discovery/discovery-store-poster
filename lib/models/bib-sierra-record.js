...
class BibSierraRecord extends SierraRecord {
  ...
  carrierType () {
    ...
    if (this.isPartnerRecord()) return this.carrierTypeByVarField().then(fallback('nc'))
  ...
  mediaType () {
    ...
    if (this.isPartnerRecord()) return this.mediaTypeByVarField().then(fallback('n'))
  ...
  // Returns true if this bib represents an OTF (on the fly) record created in
  // Sierra to track an inbound partner item requested by one of our patrons
  isOtfRecord () {
