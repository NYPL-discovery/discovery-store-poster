...
class SierraRecord {
   ...
   *
   * @param {string} marc - Marc tag, e.g. '700'
   * @param {array|string|null} subfields - Subfield (e.g. 'a') or array of
   *    subfields (e.g. ['a', 'z']) to match, if any specifically.
   * @param {QueryOptions} options - See QueryOptions below
   */
  ...
  /**
   * @typeobj {object} QueryOptions
   * @property {boolean} tagSubfields - Controls whether queried values should
   *    be returned as a hash of values (with subfields as keys) or a single
   *    joined string
   * @property {boolean} joinSubfields - Controls whether subfields within a
   *    varField should be joined together using `subfieldJoiner`. Default true
   *    Typically used to get each instance of a single queried subfield
   *    that repeats within a varfield. Setting joinSubfields=false when
   *    querying more than one subfield isn't an anticipated use case.
   * @property {string} subfieldJoiner} - String to join subfields in a given
   *    varfield match together. Default ' '
   * @property {function} preFilter - Function to use to use to filter out
   *    passed `varFields` before processing. Default: ()=>true
   */

  /**
   * For an array of varfield blocks of interest (assumed already filtered by
   * tag), returns an array of extracted values.
   *
   * @param {array<object>} varfields - Array of matched varFields to query into
   * @param {array|string|null} subfields - Subfield (e.g. 'a') or array of
   *    subfields (e.g. ['a', 'z']) to match, if any specifically.
  ...
  _extractValuesFromVarFields (varFields, subfields, opts) {
    ...
    if (varFields.length) {
        ...
        var subfieldParse = (subs) => {
          if (opts.tagSubfields) {
            return subs.reduce((hash, sub) => {
              // If subfield repeats, add joiner:
              if (hash[sub['tag']]) hash[sub['tag']] += opts.subfieldJoiner + sub['content']
              else hash[sub['tag']] = sub['content']

              return hash
            }, {})
          } else {
            const values = subs.map((sub) => sub['content'])
            if (opts.joinSubfields) return values.join(opts.subfieldJoiner)
            else return values
          }
        }
      ...
      // Now that vals is an array of extracted values for each varfield,
      // return a flattened array:
      return [].concat.apply([], vals)
        // .. and filter it by truthyness (to remove blanks)
        .filter((v) => v)
    } else return []
  }
}
