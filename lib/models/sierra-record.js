...
class SierraRecord {
  ...
  _extractValuesFromVarFields (varFields, subfields, opts) {
    ...
    console.log('extracting varFieldz: ', JSON.stringify(opts, null, 2), JSON.stringify(varFields, null, 2))

    if (varFields.length) {
      var vals = varFields.filter(opts.preFilter).map((f) => {
        // sometimes there's a case error...
        var unfilteredFields = f.subFields || f.subfields
        var _subFields = unfilteredFields && opts.excludedSubfields
          ?
