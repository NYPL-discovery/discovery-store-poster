  ...
  _deferredConnect (checkNumber) {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        ...
        else if (checkNumber >= CONNECTION_DEFER_MAX_CHECKS) return reject('Deferred connect timeout')
        // Recurse!
...
var createDb = (recreate) => {
  var _create = () => {
    ...
    // Add resource_statement objectid-by-predicate index:
    creators.push(execute('CREATE INDEX resource_predicate_object_id ON resource_statement (predicate, object_id) WHERE predicate IN (\'rdf:type\', \'nypl:bnum\')'))
