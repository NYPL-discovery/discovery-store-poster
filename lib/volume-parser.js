...
exports.parseVolume = (fieldTagV) => {
  ...
  if (!volumes.length && fieldTagV.match(singletonRegex)) {
    const vol = parseInt(fieldTagV.match(singletonRegex)[0], 10)
    console.log(fieldTagV.match(singletonRegex))
    volumes = [[vol, vol]]
  }
  return volumes
