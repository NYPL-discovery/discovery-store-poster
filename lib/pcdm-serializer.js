  ...
  serialize () {
    return FieldMapping.initialize('resources').then((fieldMapper) => {
      try {
        ...
        var branchOrResearch = itemType && itemType.match(/^\d+$/) && parseInt(itemType) <= 100 ? 'research' : 'branch'
        ...
        // Callnum
        var callnum = this.object.callNumber
        if (!callnum && this.object.varField('852', ['h'])) callnum = this.object.varField('852', ['h'])[0]
  ...
  serialize () {
    return FieldMapping.initialize('resources').then((fieldMapper) => {
      try {
        ...
        var langCodes = []
        if (this.object.lang) {
          langCodes = utils.flattenArray([this.object.lang]).map((lang) => lang.code)
        } else {
        ...
        promises.push(MarcMapping.initialize().then((marcMapper) => {
          marcMapper.allMappings().filter((mapping) => {
            ...
            return ['Carrier type', 'Type', 'Availability', 'Barcode', 'Call number', 'Date', 'Date created', 'Date start', 'Date end', 'Language'].indexOf(mapping.name) < 0
          }).map((mapping) => {
            ...
            mapping.marc.forEach((marc) => {
              ...
              if (values && values.length) values.forEach((value) => this.addStatement(mapping.predicate, { objectLiteral: value, label: mapping.description }), provo)
            })
          })
        ...
        promises.push(Promise.all(urls.map(utils.imageDimensions)).then((sizes) => {
