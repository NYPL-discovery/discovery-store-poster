  ...
  serialize () {
    return FieldMapping.initialize('resources').then((fieldMapper) => {
      try {
        ...
        this.addStatement('rdf:type', { objectUri: 'nypl:Item' }, provo)
        ...
        _([this.object.bibIds]).flatten().compact().each((bibId) => {
          ...
          this.addStatement('nypl:bnum', { objectUri: `urn:bnum:${sourcePrefix(this.object)}b${bibId}` }, provo)
        })
        ...
        if (this.object.electronicResources) {
          this.object.electronicResources.forEach((r) => {
            this.addStatement('nypl:electronicLocator', { objectLiteral: r.url, label: r.label }, provo)
          })
        }
        ...
        this.addStatement('nypl:itemType', { objectUri: `urn:itemtype:${branchOrResearch}` }, provo)
        ...
        // Callnum
        var callnum = null
        log.debug('Call num: ', this.object.callNumber, this.object.varField('852', ['h']))
        ...
        if (statusMatch) {
          console.log('now here -1', statusMatch)
  ...
  serialize () {
    return FieldMapping.initialize('resources').then((fieldMapper) => {
      try {
        ...
        this.addStatement('rdf:type', { objectUri: 'nypl:Bib' }, provo)
        ...
        promises.push(PcdmBibSerializer.parseElectronicResources(this.object).then((resources) => {
          ...
          resources.forEach((resource) => {
            this.addStatement('bf:supplementaryContent', { objectLiteral: resource.url, label: resource.label }, provo)
          })
        }))
        ...
        promises.push(MarcMapping.initialize().then((marcMapper) => {
          ...
          ; ['600', '610', '611', '630', '648', '650', '651', '653', '655'].forEach((marc) => {
            ...
            if (vals) {
              ...
              ; vals.forEach((val) => this.addStatement(predicate, { objectLiteral: val }, provo))
            }
          })
...
module.exports = { PcdmBibSerializer, PcdmItemSerializer, PcdmOrganizationSerializer }
