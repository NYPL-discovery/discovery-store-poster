...
class ItemsUpdater extends UpdaterBase {
  ...
  _writeToIndexDocumentQueue (batch) {
    log.debug(`_writeToIndexDocumentQueue called with a batch size of ${batch.length} that looks like ${JSON.stringify(batch)}`)
    ...
    if (streamName && schemaName) {
      ...
      log.debug(`_writeToIndexDocumentQueue will operate on the distinctBibIds: ${distinctBibIds}`)
      // Submit bib ids to write stream:
      var recs = distinctBibIds.map((uri) => {
        log.debug(`mapping over URI: ${uri}`)
        return { type: 'record', uri }
      })

      log.debug(`_writeToIndexDocumentQueue mapped the distinctBibIds, to recs ${JSON.stringify(recs)}`)
      ...
      return this._writeToStreamsClient(streamName, recs, schemaName)
        ...
        .catch((error) => {
          log.error('can\'t write to streams client', error.stack)
        })
