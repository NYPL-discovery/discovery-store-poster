...
class ItemsUpdater {
  ...
  update (options, data) {
    ...
    var nonstreamItem = ItemSierraRecord.from(JSON.parse(data));

    console.log('got item, about to process.');
    // return this._processItemStream(stream, options)
    return this._processItemKinesis(nonstreamItem, options);
  }
  ...
  _processItemStream (stream, options) {
    ...
    stream
      ...
      .map((r) => db.upsertStatements('resource', r.statements).then(() => ({ count: r.statements.length, item: r.item })))
      .flatMap((r) => _(r))
      .map((batch) => {
        var num = 1 // batch && (typeof batch.length) === 'number' ? batch.length : 1
        log.info(`ItemsUpdater#update: Saved batch of ${num}: ${batch.item.id} (${totalProcessed} total processed, index ${totalProcessed + options.offset})`)
