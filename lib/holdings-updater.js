...
class HoldingsUpdater extends UpdaterBase {
    ...
    return this.statementsForHolding(holding)
      .catch((e) => {
        ...
        return this.saveToDisc(holding).then((r) => {
          ...
          console.log(`  Run with --uri_cache ${holding.id} to test again`)
        })
      })
  }
  ...
  _getBibIdsForHoldingStatements (statements) {
    ...
    const subjectIdsToBibIds = statements.reduce((hash, statement) => {
      if (Object.keys(hash).includes(statement.subject_id) && hash[statement.subject_id] != null) continue
      else if (statement.predicate === 'nypl:bnum') hash[statement.subject_id] = statement.object_id.replace('urn:bnum:', '')
      else hash[statement.subject_id] = null
    })
...
HoldingsUpdater.threaded = (opts) => {
  if (cluster.isMaster) {
    ...
    var holdingsSource = SourceLoader.load(opts.holdingsSource)
    ...
    if (opts.limit) {
      ...
      holdingsSource.count().then((count) => {
    ...
    process.on('message', (msg) => {
      ...
      kmsHelper.decryptDbCreds().then((dbConnectionString) => {
        ...
        var holdings = holdingsSource.stream().slice(msg.start, msg.start + msg.total)
        ; (new HoldingsUpdater()).update(Object.assign({}, events, opts), holdings)
      })
    })
  }
}
